...
- `/vendor`: scoped access, vendor matching profile, lifecycle tracker,
  bid/price context, closeout snapshot, request readiness, guided owner-handoff
  drafts, status handoff guidance, updates, proof upload, and completion-waiver
  path make vendor next actions obvious.
